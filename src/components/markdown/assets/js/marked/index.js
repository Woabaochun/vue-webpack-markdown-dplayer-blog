...
var inline = {
  ...
  video: /^H?\{(label)\}\(\s*(href)(?:\s+(title))?\s*\)/,
  reflink: /^!?\[(label)\]\[(?!\s*\])((?:\\[\[\]]?|[^\[\]\\])+)\]/,
  nolink: /^!?\[(?!\s*\])((?:\[[^\[\]]*\]|\\[\[\]]|[^\[\]])*)\](?:\[\])?/,
...
inline.video = edit(inline.video)
  .replace('label', inline._label)
  .replace('href', inline._href)
  .replace('title', inline._title)
  .getRegex();
...
InlineLexer.prototype.outputVideo = function(cap, video, src) {
  var href = video.href,
    title = video.title ? escape(video.title) : null;
  return cap[0].charAt(0) !== 'H' ?
...
//     return text;
//   }
//   var out = '<up-dplayer style="width: 100%;min-height: 500px;" :file-url="'
//   + href + '" id="' + text + '"';
...
// TextRenderer.prototype.video =
//     TextRenderer.prototype.video = function (href, title, text) {
//         return '' + text;
//     };
...
Parser.prototype.tok = function() {
  if (InlineLexer.rules.video.exec(this.token.text)) {
    // debugger
    this.token.type = 'html';
  }
    ...
    case 'html':
      {
        // debugger
        var cap = '';
        if (cap = InlineLexer.rules.video.exec(this.token.text)) {
          // debugger
          this.token.type = 'html';
          this.token.escaped = 'video';
          // debugger
          this.token.text = cap[2];
          // debugger
          // this.token.text = cap[0]
          return this.renderer.html(this.token.text, 'video');
        }
